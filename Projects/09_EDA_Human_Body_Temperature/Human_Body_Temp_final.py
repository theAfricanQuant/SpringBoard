...
def one_sample_test(xvector,mu):
    ...
    print(f'The p-value for the t-statistic is: {str(p_val)}')
    if p_val < 0.05:
        print(
            f'We can reject the null hypothesis; The mean IS different than {str(mu)}'
        )
    else:
        print(
            f'We cannot reject the null hypothesis; The mean is NOT different than {str(mu)}'
        )
...
def expected_rante(observations):
    ...
    print(f'The lower limit at the 95% conf. level is {str(lb)}')
    print(f'The upper limit at the 95% conf. level is {str(ub)}')
    print(10*'--')
    return lb, ub

def men_vs_women(fem,mal):
    _, p = ttest_ind(fem,mal,equal_var=False)
    print(f'The p-value for the t-statistic is: {str(p)}')
    if p < 0.05:
